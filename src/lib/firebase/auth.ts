...
export async function updateProfile(user: User, displayName?: string, photoURL?:string){
  await _updateProfile(user, {displayName, photoURL})
}

export async function signInWithEmailAndPassword(credentials: Credentials) {
  try {
    // console.log("Signing in with ", credentials);
    const cred = await _signInWithEmailAndPassword(
      auth,
      credentials.email || "",
